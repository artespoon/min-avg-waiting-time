...
class OrdersProcessorSpec extends Specification {
  "Test getAvgWaitingTime " >> {
    ...
    "Test with overlapping and not overlapping orders" >> {
      OrdersProcessor.getAvgWaitingTime(List(CustomerOrder(0,3), CustomerOrder(1,9), CustomerOrder(2,6), CustomerOrder(100, 8))) must_== 8
      OrdersProcessor.getAvgWaitingTime(List(CustomerOrder(0,3), CustomerOrder(1,9), CustomerOrder(2,5), CustomerOrder(100, 8))) must_== 8
    }
  }
}
